...
impl PriceGenerator {
    ...
    pub async fn run(mut self, shutdown: triggered::Listener) -> Result<()> {
        match (self.key, self.default_price) {
            (Some(key), _) => self.run_with_key(key, &shutdown).await,
            (None, Some(defaut_price)) => self.run_with_default(defaut_price, &shutdown).await,
            _ => {
                tracing::warn!(
    ...
    async fn run_with_key(&mut self, key: SolPubkey, shutdown: &triggered::Listener) -> Result<()> {
