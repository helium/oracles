...
impl Server {
    ...
    async fn handle_poc_tick(&mut self) -> Result {
        ...
        tracing::info!("updating meta last_poc_submission_ts to: {:?}", before_utc);
...
async fn submit_txns(
    ...
    let file_list = store
        ...
        .await?;
    tracing::info!("file_list len: {:?}", file_list.len());
    ...
    store
        ...
        .try_for_each_concurrent(STORE_WORKERS, |msg| {
            ...
            async move {
                ...
                if do_submission {
                    tracing::info!("submitting txn, hash: {:?}", txn_details.hash_b64_url);
                    handle_txn_submission(txn_details.clone(), &mut shared_txn_service).await?;
                    tracing::info!("storing txn, hash: {:?}", txn_details.hash_b64_url);
