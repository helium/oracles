    ...
    AV: AuthorizationVerifier + Send + Sync + 'static,
{
    ...
    async fn submit_ban(&self, request: Request<BanReqV1>) -> GrpcResult<BanRespV1> {
        ...
        _ = self.ban_sink.write(report, []).await;
...
pub async fn grpc_server(settings: Settings) -> Result<()> {
    ...
    let Some(api_token) = format!("Bearer {}", settings.token)
    ...
    tracing::info!(
        "grpc listening on {} and server mode mobile",
        settings.listen_addr,
    );
