...
pub struct Cmd {
    /// Process name for tracking unique_connections backfill (avoids conflict with daemon)
    #[clap(long, default_value = "unique-connections-backfill")]
    process_name: String,
...
impl Cmd {
    pub async fn run(self, settings: &Settings) -> Result<()> {
        let pool = settings
            .database
            .connect("mobile-verifier-unique-connections-backfill")
            .await?;
        ...
        tracing::info!(
            ...
            "starting unique_connections backfill"
        );
