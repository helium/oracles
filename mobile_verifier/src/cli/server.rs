...
impl Cmd {
    pub async fn run(self, settings: &Settings) -> Result<()> {
        ...
        // Heartbeats
        let (heartbeats_receiver, heartbeats_server) =
        ...
        // Speedtests
        let (speedtests_receiver, speedtests_server) =
        ...
        let rewarder_daemon = Rewarder::new(
            pool.clone(),
        ...
        // subscriber location
        let (subscriber_location_ingest_receiver, subscriber_location_ingest_server) =
        ...
        let subscriber_location_ingestor_daemon = SubscriberLocationIngestor::new(
            pool.clone(),
            auth_client.clone(),
            entity_client.clone(),
            subscriber_location_ingest_receiver,
            verified_subscriber_location,
        );

        // data transfers
        let (data_session_ingest_receiver, data_session_ingest_server) =
        ...
        let data_session_ingestor_daemon = DataSessionIngestor::new(pool.clone(), data_session_ingest_receiver);
        ...
        .add_task(heartbeats_server)
        .add_task(speedtests_server)
        .add_task(subscriber_location_ingestor_daemon)
        .add_task(data_session_ingestor_daemon)
        .add_task(price_daemon)
        .add_task(heartbeat_daemon)
        .add_task(speedtest_daemon)
        .add_task(rewarder_daemon)
        .start()
        .await
