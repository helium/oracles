...
pub struct Cmd {
    /// Process name for tracking ban backfill (avoids conflict with daemon)
    #[clap(long, default_value = "ban-backfill")]
    process_name: String,
...
impl Cmd {
    pub async fn run(self, settings: &Settings) -> Result<()> {
        let pool = settings
            .database
            .connect("mobile-verifier-ban-backfill")
            .await?;
