...
/// Remove a gateway from the inventory table.
///
/// The tests below need to tell "answered from the shared snapshot" apart from
/// "answered by a fallback query", and the two agree whenever the row is in the
/// table. Deleting it after a refresh splits them: the snapshot still holds the
/// entry, the fallback finds nothing.
///
/// This replaces an older trick that asked at a timestamp before the gateway
/// existed, on the reasoning that only the fallback would answer `false`. That
/// stopped working when the snapshot became timestamp-aware (#1230) and both
/// paths started answering `false` together.
async fn remove_from_inventory(
    ...
    trino_client::Client::from_client(harness.owned_trino().await?)
        .execute_raw(format!(
            "DELETE FROM {} WHERE pub_key = '{gateway}'",
            hotspot_inventory::RESOLVER_TABLE
        ))
        .await?;
    Ok(())
...
async fn a_clone_sees_gateways_added_after_it_was_taken() -> anyhow::Result<()> {
    ...
    // Now take it back out of the table. Only the snapshot still knows about it,
    // so a clone holding its own stale copy would miss, fall through to a
    // per-pubkey query, and find nothing.
    remove_from_inventory(&harness, &onboarded_later).await?;

    assert!(
        clone.is_gateway_known(&onboarded_later, &Utc::now()).await,
...
async fn an_independent_resolver_does_not_share_a_snapshot() -> anyhow::Result<()> {
    ...
    assert!(
        resolver
            .is_gateway_known(&onboarded_later, &Utc::now())
            .await,
