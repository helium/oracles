...
async fn test_confirm_pending_txns(pool: PgPool) -> anyhow::Result<()> {
    ...
    let valid_sessions = FileSinkClient::new(valid_sessions_tx, "test");
    pending_burns::confirm_pending_txns(&pool, &solana_network, &valid_sessions).await?;
...
fn confirming_pending_txns_writes_out_sessions(pool: PgPool) -> anyhow::Result<()> {
    ...
    assert_eq!(payer_burn.pubkey, pubkey_one);
    assert_eq!(
        payer_burn.total_dcs,
        pending_burns::bytes_to_dc(5_000),
        "total dcs do not match still"
    );
...
#[sqlx::test]
fn will_not_burn_when_pending_txns(_pool: PgPool) -> anyhow::Result<()> {
    // Trigger a burn when there is pending burns _and_ pending txns.
