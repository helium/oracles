...
pub async fn grpc_server(settings: Settings) -> anyhow::Result<()> {
    let auth_key = PublicKey::from_str(&settings.chain_rewardable_entities_auth_key)?;
    ...
    tracing::info!(
        "grpc listening on {} and server mode chain",
...
impl ManagedTask for GrpcServer {
    fn start_task(
        ...
        task_manager::spawn(
            Server::builder()
                // TODO(mj): mismatching http versions?
...
fn make_span(_request: &http::Request<tonic::body::Body>) -> tracing::Span {
