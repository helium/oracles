...
mod tests {
    ...
    async fn write_idempotent_skips_duplicate_id_across_instances() -> anyhow::Result<()> {
        ...
        // Trino `map[key]` raises on missing keys; `element_at` returns
        // NULL instead, which is what we want for snapshots that were
        // plain `write` calls (no write_id stamped).
        let counts = harness
            .trino()
            .get_all::<Count>(
    ...
    ///
    /// Uses a multi-threaded runtime, two independent `IcebergTable`
    /// instances (so each has its own metadata cache), and a barrier to
    /// force both writers to reach their commit call simultaneously. One
    /// will win the `RefSnapshotIdMatch` optimistic lock, the other gets a
    /// 409 and its `do_commit` refreshes and re-applies fast_append against
    /// the new main.
    ///
    /// Repeats the race several times to shake out scheduling-dependent
    /// flakes — if any iteration lost a row, the assertion fires.
    ...
    async fn concurrent_writes_dont_clobber() -> anyhow::Result<()> {
        ...
        for round in 0..ROUNDS {
            // Two independent writer instances — each calls from_catalog,
            // so each has its own Arc<RwLock<Table>> metadata cache. This
            // is the critical difference from cloning a single BoxedDataWriter.
            let writer_a: Arc<dyn crate::DataWriter<Person>> =
                harness.get_table_writer::<Person>("people").await?;
            let writer_b: Arc<dyn crate::DataWriter<Person>> =
                harness.get_table_writer::<Person>("people").await?;
            ...
            let task_a = tokio::spawn(async move {
                // Wait until both tasks are ready so they hit the commit
                // path as close to simultaneously as possible.
