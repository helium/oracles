...
lazy_static! {
    /// the duration in seconds after when a beacon report in the DB will be deemed stale
    /// this value will be added to the setting BASE_STALE_PERIOD to determine final setting
    pub static ref BEACON_STALE_PERIOD: Duration = Duration::minutes(45);
    /// the duration in seconds after when a witness report in the DB will be deemed stale
    /// this value will be added to the setting BASE_STALE_PERIOD to determine final setting
    static ref WITNESS_STALE_PERIOD: Duration = Duration::minutes(45);
    /// the duration of time in seconds after which entropy will be deemed stale
    /// this duration should always be greater than BEACON_STALE_PERIOD
    /// otherwise valid beacons will potentially end up being marked as stale
    /// this value will be added to the setting BASE_STALE_PERIOD to determine final setting
...
pub struct Purger {
    ...
    store_output: file_store::Settings,
...
impl Purger {
    pub async fn from_settings(settings: &Settings) -> Result<Self, NewPurgerError> {
        ...
        let store_base_path = settings.cache.clone();
        let store_output = settings.output.clone();
        Ok(Self {
            pool,
            store_base_path,
            store_output,
    ...
    pub async fn run(&self, shutdown: &triggered::Listener) -> anyhow::Result<()> {
        ...
        let store_base_path = Path::new(&self.store_base_path);
        let (file_upload_tx, file_upload_rx) = file_upload::message_channel();
        let file_upload =
            file_upload::FileUpload::from_settings(&self.store_output, file_upload_rx).await?;
