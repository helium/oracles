...
latlong = { "pacific": (Decimal(35.160337), Decimal(204.104148)),
...
for x in re.split(',', os.environ['5G_GATEWAYS']):
    ...
    # consistent hash
    i = (crc32(x.encode()) & 0xffffffff) % len(latlong)
    # between 1 and 3 cell ids
    cell_ids = [ randrange(10000, 99999) for _ in xrange(max(1, min(3, (i % 3)+1))) ]
    # use the index to select the lat, long coordinates
    (lat, lon) = latlong[list(latlong)[i]]
    gateways.append({ 'gw_addr': x, 'lat': lat, 'lon': lon, 'cell_ids': cell_ids })
...
for gw in gateways:
    ...
    hdrs = { 'User-Agent': 'heartbeat-daemon-1',
             'API-Key': os.environ['5G_API_KEY'] }
    req = urllib.request.Request(os.environ['5G_HEARTBEAT_URL'], data=data, headers=hdrs, method='POST')
    with urllib.request.urlopen(req, timeout=30, context=context) as resp:
        status = resp.get_code()
        if status > 199 and status < 300:
            log.info("{} status for gw: {}".format(status, heartbeat['pubkey']))
        else:
            log.error("{} status request data {}".format(status, pformat(req)))
