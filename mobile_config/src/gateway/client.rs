...
use crate::gateway::info::{self, GatewayInfo, GatewayInfoStream};
...
pub struct GatewayClient {
    ...
    batch_size: u32,
    cache: Arc<Cache<PublicKeyBinary, Option<info::GatewayInfo>>>,
    cache_ttl: Duration,
}
...
impl GatewayInfoResolver for GatewayClient {
    async fn resolve_gateway_info(
        &self,
        address: &PublicKeyBinary,
    ) -> Result<Option<info::GatewayInfo>, ClientError> {
        ...
        let response = match call_with_retry!(self.client.clone().info_v2(request.clone())) {
            Ok(info_res) => {
                ...
                response.info.map(info::GatewayInfo::try_from).transpose()?
            }
    ...
    async fn stream_gateways_info(
        &mut self,
        device_types: &[DeviceType],
    ) -> Result<info::GatewayInfoStream, ClientError> {
