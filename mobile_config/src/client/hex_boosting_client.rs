...
pub trait HexBoostingInfoResolver: Send + Sync {
...
impl HexBoostingInfoResolver for HexBoostingClient {
    async fn stream_boosted_hexes_info(&self) -> Result<BoostedHexInfoStream, ClientError> {
        ...
        let res_stream = call_with_retry!(client.info_stream(req.clone()))?
            .into_inner()
    ...
    async fn stream_modified_boosted_hexes_info(
        ...
        let pubkey = Arc::new(self.config_pubkey.clone());
        let mut client = self.client.clone();
        let res_stream = call_with_retry!(client.modified_info_stream(req.clone()))?
            .into_inner()
            .filter_map(|res| async move { res.ok() })
            .map(move |res| (res, pubkey.clone()))
            .filter_map(|(res, pubkey)| async move {
