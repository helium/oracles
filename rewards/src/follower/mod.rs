pub mod client;
pub mod follower_meta;
pub use client::FollowerService;
pub use follower_meta::FollowerMeta;
...
impl Follower {
    ...
    pub async fn run(&mut self, shutdown: triggered::Listener) -> Result {
        ...
        loop {
            ...
            let height = FollowerMeta::last_height(&self.pool, self.start_block).await? as u64;
            ...
            tokio::select! {
                msg = txn_stream.message() => match msg {
                    Ok(Some(txn)) => {
                        ...
                        FollowerMeta::update_last_height(&self.pool, height).await?;
                    }
                    Ok(None) => {
    ...
    async fn process_consensus_group(&mut self, envelope: &FollowerTxnStreamRespV1) -> Result {
        ...
        match self
            .trigger
            .send(ConsensusTxnTrigger::new(ht as i64, ts as i64))
        {
            Ok(_) => Ok(()),
            Err(_) => {
